...
public class ChatMiniController {
    ...
    public DataResult<List<FriendRespVO>> friends(@PathVariable("type")String type){
        ...
        MpUser mpUser = mpUserService.findByUnionId(unionid);
        //获取个人的id
        MineRespVO mineRespVO = friendService.getByUidAndType(mpUser,type.toLowerCase());
        //获取好友列表
    ...
    public DataResult<Integer> chatByDid(@PathVariable("did")Integer did){
        ...
        MineRespVO friend =  friendService.getByUidAndType(doctor.getMpUser(),"pc");
        if(null==friend){
            throw new BusinessException(BaseExceptionType.MINI_ERROR,"对话对象不存在");
        }
    ...
    public DataResult<Integer> chatByIid(@PathVariable("iid")Integer iid){
        ...
        DataResult<Integer> result = DataResult.success();
        MineRespVO mineRespVO =  friendService.getByUidAndType(information.getMpUser(),"mobile");
        if(null==mineRespVO){
            throw new BusinessException(BaseExceptionType.MINI_ERROR,"对话对象不存在");
        }
